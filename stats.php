<?php
    ...
    //Get total-storages numbers
        $question = mysqli_query($conn, "SELECT COUNT(storageId) AS numberOfStorages FROM storages;");
        $stats[] = array("totalStorages"=>mysqli_fetch_row($question)[0]);
    
    //Get total-categories number
        $question = mysqli_query($conn, "SELECT COUNT(categoryId) AS numberOfCategories FROM categories;");
